Vec2D[] curveToArray(ArrayList<Vec2D> curve) {
...
Vec2D[] densityResample(ArrayList<Vec2D> curve, float linearDensity) {
  return densityResample(curveToArray(curve), linearDensity);
}

Vec2D[] regularResample (ArrayList<Vec2D> curve, int newLen) {
  return regularResample(curveToArray(curve), newLen);
}

Vec2D[] resample (ArrayList<Vec2D> curve, int newLen) {
  return resample(curveToArray(curve), newLen);
}
